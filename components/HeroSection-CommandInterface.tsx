...
export default function CommandInterface() {
  ...
  const thinkingSteps = [
    ...
    { text: "Querying financial database", icon: Database },
    { text: "Processing 2.3M data points", icon: Activity },
    { text: "Generating insights", icon: Sparkles }
  ]

  const answerText = "Based on your real-time financial data, your current monthly burn rate is **$487,000**, which represents a **12% improvement** from last quarter. The reduction is primarily driven by optimized cloud infrastructure costs (-$43,000) and improved vendor negotiations (-$28,000). At this rate, you have **18.4 months of runway** with current cash reserves."
  ...
  const startStreamingText = () => {
    ...
    const streamInterval = setInterval(() => {
      ...
      setStreamedText(answerText.slice(0, index + 1))
      index += 3 // Speed of streaming
    }, 10)
  }

  const animateComponents = () => {
    statsComponents.forEach((_, index) => {
      setTimeout(() => {
    ...
    }, statsComponents.length * 100 + 300)
  }

  const resetToWelcome = () => {
    console.log('Resetting to welcome state')
    // Reset all states
            ...
            <div style={styles.answerContent}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <h3 style={styles.answerTitle}>Financial Analysis</h3>
                </div>
                <motion.button
                  onClick={(e) => {
                    e.preventDefault()
              ...
              >
                ...
                {streamedText.length < answerText.length && (
                  <span style={styles.cursor} />
                )}
              </motion.div>

              {/* Rich Components - Staggered Animation */}
              {showComponents && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
              ...
              {showFollowups && (
                ...
                >
                  ...
                  <div style={styles.followupsGrid}>
                    {followupPrompts.map((followup, index) => (
                      <motion.button
                        key={index}
